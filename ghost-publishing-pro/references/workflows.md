...
```bash
CREDS="$HOME/.openclaw/credentials/ghost-sites.json"
KEY=$(node -e "const c=JSON.parse(require('fs').readFileSync('$CREDS','utf8'));process.stdout.write(c.sites['primary'].key)")
SITE_URL=$(node -e "const c=JSON.parse(require('fs').readFileSync('$CREDS','utf8'));process.stdout.write(c.sites['primary'].url)")
ID="${KEY%%:*}"; SECRET="${KEY#*:}"
NOW=$(date +%s); EXP=$((NOW+300))
H=$(printf '{"alg":"HS256","typ":"JWT","kid":"%s"}' "$ID" | openssl base64 -A | tr '+/' '-_' | tr -d '=')
P=$(printf '{"iat":%d,"exp":%d,"aud":"/admin/"}' "$NOW" "$EXP" | openssl base64 -A | tr '+/' '-_' | tr -d '=')
S=$(printf '%s.%s' "$H" "$P" | openssl dgst -sha256 -mac HMAC -macopt "hexkey:$SECRET" -binary | openssl base64 -A | tr '+/' '-_' | tr -d '=')
TOKEN="$H.$P.$S"
# Change 'primary' to 'secondary' for the other site
```
...
URL=$(node -e "const c=JSON.parse(require('fs').readFileSync('$CREDS','utf8'));process.stdout.write(c.url)")
ID="${KEY%%:*}"; SECRET="${KEY#*:}"
NOW=$(date +%s); EXP=$((NOW+300))
H=$(printf '{"alg":"HS256","typ":"JWT","kid":"%s"}' "$ID" | openssl base64 -A | tr '+/' '-_' | tr -d '=')
P=$(printf '{"iat":%d,"exp":%d,"aud":"/admin/"}' "$NOW" "$EXP" | openssl base64 -A | tr '+/' '-_' | tr -d '=')
...
```bash
# TOKEN and URL exported from Step 1
node -e "
const https=require('https');

const token=process.env.TOKEN;
const creds={url:process.env.URL};

function makeToken(){ return token; }
...
Save as `ghost-performance.js` and run with:

```bash
# Generate token first (bash + openssl — see Authentication section)
source scripts/ghost-token.sh
TOKEN=$TOKEN GHOST_URL=$GHOST_URL node ghost-performance.js
```
...
// Token and URL passed in via environment (generated by ghost-token.sh)
const ghostToken = process.env.TOKEN;
const creds = { url: process.env.GHOST_URL };
